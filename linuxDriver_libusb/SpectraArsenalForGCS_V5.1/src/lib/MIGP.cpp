...
WORD PrintfDebugSwitch = AS_DLL_PRINTF_DEBUG_OFF;
...
BOOL MIGP_Frame_Resolution(void *pstCommHandle, BYTE **bBuff, BYTE bCMD, BYTE bCRCFlag)
{
	...
	{
//		printf_debug("MIGP_Frame_Resolution::pstCommHandle is NULL");
		return FALSE;
	}
...
//    	printf_debug("MIGP_Frame_Resolution::pstMigpPara->stMIGP.ulBuffTxNum is wrong");
    ...
    *bBuff = &pstMigpPara->stMIGP.bFrameRxBuff[3];
	...
	if(pstMigpPara->stMIGP.bMasterAddress != pstMigpPara->stMIGP.bFrameRxBuff[0])
	{
		...
		{
//			printf_debug("MIGP_Frame_Resolution::address is wrong");
	        return FALSE;
		}
	}
    ...
    if(ulLength != 0)
    {
    	if(bCRCFlag == 1)
    	{
	        ...
	        {
//	        	printf_debug("MIGP_Frame_Resolution::LRC Check False");
	            return FALSE;
	        }
    	}
    }
	...
	{
//		printf_debug("MIGP_Frame_Resolution::length is 0");
        return FALSE;
	}

	if(RxCMD != (bCMD | 0x80))
	{
//		printf_debug("MIGP_Frame_Resolution:: Rx CMD is wrong");
		return FALSE;
	}
...
//	int ulDataLength;
	...
	if(pstMigpPara->enCommType == COM_SERIAL)
	{
		PurgeComm(pstMigpPara->stComPara.ComHandle,  PURGE_RXCLEAR | PURGE_TXCLEAR);
		
		Com_Write(pstMigpPara->stComPara.ComHandle, pstMigpPara->stMIGP.bFrameTxBuff, pstMigpPara->stMIGP.ulBuffTxNum);
	}
...
//	int ulDataLength;
	...
	bTemp[6] = (BYTE)((ulLength >> 8) & 0x000000ff);
	bTemp[7] = (BYTE)(ulLength & 0x000000ff);
	for(i = 0; i < ulParaLength; i++)
	{
		bTemp[8 + i] = pbParaData[i];
	}
	MIGP_Construct_FrameData(pstMigpPara->stMIGP.bFrameTxBuff, &pstMigpPara->stMIGP.ulBuffTxNum, 8 + ulParaLength, bTemp, &bLRC);
